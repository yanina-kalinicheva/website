<footer class="footer-widgets">
  <div class="container">
	<div class="row section" style ="padding-top: 50px; padding-bottom: 50px;">
	  <!-- Contacts -->
	  <div class="col-md-6 col-sm-4 mb-sm-100" style="margin-bottom:0px">
		<div class="widget about-widget">
		  <h5 class="header-widget">Контакты</h5>
		  <p>11049 Тайвань, Тайбэй, ул. Синьи, секция 5, номер 2, этаж 9. Тел. (+886-2) 8780-3011, факс (+886-2) 8780-2511, эл.почта info@mtc.org.tw</p>
		  <a href="contacts.php">Карта</a>

		  <ul class="social-links">
			<li><a href="https://www.facebook.com/mtcintaipei"><i class="fa fa-facebook"></i></a></li>
			<li><a href="https://twitter.com/mtcintaipei"><i class="fa fa-twitter"></i></a></li>
		  </ul>
		</div><!-- / .widget -->
	  </div><!-- / .col-md-3 -->

	  <!-- Twitter Feed -->
	  <div class="col-md-6 col-sm-4 mb-sm-100" style="margin-bottom:0px">
		<div class="widget twitter-widget">
		  <h5 class="header-widget">Лента Твиттера</h5>
		  <ul>
			
			<a class="twitter-timeline" href="https://twitter.com/mtcintaipei"  data-width="500"  
			data-height="200" data-theme="dark">Tweets by mtcintaipei</a> <script async src="//platform.twitter.com/widgets.js" charset="utf-8"></script>		

		  </ul>
		</div><!-- / .widget -->
	  </div><!-- / .col-md-3 -->


	</div><!-- / .row -->
  </div><!-- / .container -->

  <!-- Copyright -->
  <div class="copyright">
	<div class="container">
	  <div class="row">
		
		<div class="col-sm-9" style="font-size: 10px">
		  <small>Copyright © 1996-2017. Все права защищены. При использовании материалов ссылка на сайт Представительства МТК обязательна.<br>
				 Дизайн, разработка и поддержка сайта: yanina.kalinicheva@gmail.com<br> 
				 </small>
		</div>

		<div class="col-sm-3">
		  <small><a href="#page-top" class="pull-right to-the-top">Вверх<i class="fa fa-angle-up"></i></a></small>
		</div>

	  </div><!-- / .row -->
	</div><!-- / .container -->
  </div><!-- / .copyright -->

</footer><!-- / .footer-widgets -->